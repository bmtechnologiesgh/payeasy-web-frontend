...
export function ForgotPasswordForm() {
  ...
  async function completeReset(event: FormEvent) {
    ...
    if (json.success) {
      router.push(portalHref("ops", `/login?reset=1&email=${encodeURIComponent(email.trim().toLowerCase())}`));
      return;
    }
  ...
  return (
    <div className="w-full max-w-[440px]">
      <PageHeader
        ...
        subtitle="Enter your PayEasy staff email. We will send a one-time code so you can choose a new password."
      />
