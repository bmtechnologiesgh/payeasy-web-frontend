...
export function ForgotPasswordForm() {
  ...
  async function completeReset(event: FormEvent) {
    ...
    if (json.success) {
      router.push(
        portalHref("employer", `/login?reset=1&email=${encodeURIComponent(email.trim().toLowerCase())}`),
      );
      return;
    }
  ...
  return (
    <div className="w-full max-w-[440px]">
      <PageHeader
        ...
        subtitle="Enter your work email and we will send a one-time code so you can choose a new password."
      />
      ...
      <p className="mt-6 text-center text-sm text-[color:var(--color-muted)]">
        ...
        <Link
          href={portalHref("employer", "/login")}
          className="font-semibold text-[color:var(--color-primary)] underline-offset-2 hover:underline"
        >
          Sign in
        </Link>
