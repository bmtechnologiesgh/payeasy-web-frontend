...
import { login } from "@/lib/employer-api";
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const result = await login({
        email,
        password,
        device_name: "employer-portal",
      });
      if (!userMayAccessEmployerPortal(result.user.roles)) {
        setError("This portal is only for employer accounts. Use the merchant or operations sign-in page instead.");
        return;
      }
      setAccessToken(result.token);
      router.push(portalHref("employer", "/dashboard"));
    } catch {
  ...
  return (
    <div className="w-full max-w-[440px]">
      <PageHeader
        ...
        subtitle="Use the work email and password provided by your PayEasy administrator."
      />
            ...
            <Link
              href={
                email.trim()
                  ...
                  : portalHref("employer", "/forgot-password")
              }
